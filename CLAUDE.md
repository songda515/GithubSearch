...
6. **구현은 `/implement-spec`, 판정은 `/review-spec` — 분리된 레인.** 스펙 기반 구현/변경은 구현 스킬로
   몰고(spec-first → TCA 매핑 → TDD), GREEN 후 PR 직전 판정 스킬로 독립 검증(셀프 승인 금지)한다.
   **각 스킬의 절차·규칙은 해당 SKILL.md 가 SSoT** — 여기서 중복 서술하지 않는다(divergence 방지).
   `.claude/skills/implement-spec`, `.claude/skills/review-spec`.
...
### Screen verification — `/verify-screen`

뷰 레이어(타이틀·플레이스홀더·empty view·alert 등 화면 표시)와 spec 의 UI 상태/엣지케이스는 실제 시뮬레이터
화면으로 확인한다. **그 절차의 SSoT 는 `/verify-screen` 스킬**(`.claude/skills/verify-screen`) — 여기서 절차를
중복 서술하지 않는다. XcodeBuildMCP **설정(실행 커맨드·env)의 SSoT 는 `.mcp.json`** 이다.
...
4. **Gate before a PR: `test → review-spec → 판정 → PR`.** First run the full [Verification gates](#verification-gates--run-before-any-completion-report) (`swift test` + `xcodebuild build` + `xcodebuild test`); **if build or tests fail, do NOT commit** — fix first. Then, for spec-backed work, run **`/review-spec`** (독립 서브에이전트 판정; SDD §6) and act on the 판정: **PASS → open the PR; FAIL → 고치고 test 부터 다시.** A PR is only opened from a GREEN + PASS state.
...
- Skills (SDD §6): `implement-spec` (구현) · `review-spec` (판정) · `verify-screen` (화면검증). 각 절차의 SSoT 는 해당 `SKILL.md`. `.claude/rules` is not yet configured.
- MCP: **`.mcp.json` = XcodeBuildMCP 설정 SSoT** (실행 커맨드·env). 화면검증 절차는 `/verify-screen` 스킬 참조.
